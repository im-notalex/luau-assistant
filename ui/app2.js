...
const STORAGE_KEYS = {
  ...
  geminiKey: "rag.ui.geminiKey",
  geminiModel: "rag.ui.geminiModel",
  anthropicKey: "rag.ui.anthropicKey",
  anthropicModel: "rag.ui.anthropicModel",
  xaiKey: "rag.ui.xaiKey",
  xaiModel: "rag.ui.xaiModel",
  ...
  try {
    ...
    if (tag) {
      ...
      if (backend === "openai") {
        ...
        const custom = ($("geminiModel")?.value || "").trim();
        label = custom || "Gemini";
      } else if (backend === "anthropic") {
        const custom = ($("anthropicModel")?.value || "").trim();
        label = custom || "Anthropic";
      } else if (backend === "xai") {
        const custom = ($("xaiModel")?.value || "").trim();
        label = custom || "xAI";
...
function updateBackendVisibility() {
  ...
  setRow("geminiKeyRow", isGemini);
  setRow("geminiModelRow", isGemini);
  setRow("anthropicKeyRow", isAnthropic);
  setRow("anthropicModelRow", isAnthropic);
  setRow("xaiKeyRow", isXai);
  setRow("xaiModelRow", isXai);
...
function applyCachedSettings() {
  ...
  const anthropicModelInput = $("anthropicModel");
  const xaiKeyInput = $("xaiKey");
  const xaiModelInput = $("xaiModel");
  ...
  if (STORAGE_ENABLED) {
    ...
    const geminiKeyVal = cacheGet(STORAGE_KEYS.geminiKey);
    if (geminiKeyInput && geminiKeyVal != null) geminiKeyInput.value = geminiKeyVal;
    const geminiModelVal = cacheGet(STORAGE_KEYS.geminiModel);
    if (geminiModelInput && geminiModelVal != null) geminiModelInput.value = geminiModelVal;
    const anthropicKeyVal = cacheGet(STORAGE_KEYS.anthropicKey);
    if (anthropicKeyInput && anthropicKeyVal != null) anthropicKeyInput.value = anthropicKeyVal;
    const anthropicModelVal = cacheGet(STORAGE_KEYS.anthropicModel);
    if (anthropicModelInput && anthropicModelVal != null) anthropicModelInput.value = anthropicModelVal;
    const xaiKeyVal = cacheGet(STORAGE_KEYS.xaiKey);
    if (xaiKeyInput && xaiKeyVal != null) xaiKeyInput.value = xaiKeyVal;
    const xaiModelVal = cacheGet(STORAGE_KEYS.xaiModel);
    if (xaiModelInput && xaiModelVal != null) xaiModelInput.value = xaiModelVal;
  ...
  toggleCacheBadge("geminiKeyCached", !!(geminiKeyInput?.value || "").trim());
  toggleCacheBadge("geminiModelCached", !!(geminiModelInput?.value || "").trim());
  toggleCacheBadge("anthropicKeyCached", !!(anthropicKeyInput?.value || "").trim());
  toggleCacheBadge("anthropicModelCached", !!(anthropicModelInput?.value || "").trim());
  toggleCacheBadge("xaiKeyCached", !!(xaiKeyInput?.value || "").trim());
  toggleCacheBadge("xaiModelCached", !!(xaiModelInput?.value || "").trim());
...
function setupCacheListeners() {
  ...
  bindSecretField(geminiKeyInput, STORAGE_KEYS.geminiKey, "geminiKeyCached");
  bindSecretField(geminiModelInput, STORAGE_KEYS.geminiModel, "geminiModelCached");
  bindSecretField(anthropicKeyInput, STORAGE_KEYS.anthropicKey, "anthropicKeyCached");
  bindSecretField(anthropicModelInput, STORAGE_KEYS.anthropicModel, "anthropicModelCached");
  bindSecretField(xaiKeyInput, STORAGE_KEYS.xaiKey, "xaiKeyCached");
  bindSecretField(xaiModelInput, STORAGE_KEYS.xaiModel, "xaiModelCached");
  ...
  geminiModelInput?.addEventListener("input", () => updateStatus());
  anthropicModelInput?.addEventListener("input", () => updateStatus());
  xaiModelInput?.addEventListener("input", () => updateStatus());
  ...
  clearBtn?.addEventListener("click", () => {
    ...
    if (anthropicModelInput) anthropicModelInput.value = "";
    if (xaiKeyInput) xaiKeyInput.value = "";
    if (xaiModelInput) xaiModelInput.value = "";
    ...
    toggleCacheBadge("geminiKeyCached", false);
    toggleCacheBadge("geminiModelCached", false);
    toggleCacheBadge("anthropicKeyCached", false);
    toggleCacheBadge("anthropicModelCached", false);
    toggleCacheBadge("xaiKeyCached", false);
    toggleCacheBadge("xaiModelCached", false);
