...
app = FastAPI(title="LuaU RAG HTTP UI")
...
def api_chat(
    ...
    hosted = chosen in ('openai', 'openrouter', 'openai_compat')
...
def api_chat_stream(
    ...
    use_stream = chosen in ('ollama', 'openai', 'openrouter')
    ...
    def gen():
        ...
        try:
            if use_stream and chosen == 'ollama':
                ...
                for chunk in _oa_stream(bounded, temperature=temperature, api_key=(openai_key or None)):
                    if chat_id and _cancel_flags.get(chat_id):
                        _cancel_flags.pop(chat_id, None)
                        break
                    if chunk:
                ...
                if not had_chunk:
                    ...
                    txt, _diag = _oa_call(bounded, temperature=temperature, api_key=(openai_key or None))
                    if txt:
                        acc.append(txt)
